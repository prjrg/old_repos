...
actor_feed_forward_weights_1 = tf.Variable(tf.random.truncated_normal([INPUT_NODES, ACTOR_HIDDEN], stddev=0.01))
actor_feed_forward_bias_1 = tf.Variable(tf.constant(0.0, shape=[ACTOR_HIDDEN]))

actor_feed_forward_weights_2 = tf.Variable(tf.random.truncated_normal([ACTOR_HIDDEN, ACTIONS_COUNT], stddev=0.01))
actor_feed_forward_bias_2 = tf.Variable(tf.constant(0.1, shape=[ACTIONS_COUNT]))

actor_input_placeholder = tf.compat.v1.placeholder("float", [None, INPUT_NODES])
...
actor_action_placeholder = tf.compat.v1.placeholder("float", [None, ACTIONS_COUNT])
actor_advantage_placeholder = tf.compat.v1.placeholder("float", [None, 1])

policy_gradient = tf.reduce_mean(input_tensor=actor_advantage_placeholder * actor_action_placeholder * tf.math.log(actor_output_layer))
actor_train_operation = tf.compat.v1.train.AdamOptimizer(LEARN_RATE_ACTOR).minimize(-policy_gradient)

CRITIC_HIDDEN = 20

critic_feed_forward_weights_1 = tf.Variable(tf.random.truncated_normal([INPUT_NODES, CRITIC_HIDDEN], stddev=0.01))
critic_feed_forward_bias_1 = tf.Variable(tf.constant(0.0, shape=[CRITIC_HIDDEN]))

critic_feed_forward_weights_2 = tf.Variable(tf.random.truncated_normal([CRITIC_HIDDEN, 1], stddev=0.01))
critic_feed_forward_bias_2 = tf.Variable(tf.constant(0.0, shape=[1]))

critic_input_placeholder = tf.compat.v1.placeholder("float", [None, INPUT_NODES])
...
critic_target_placeholder = tf.compat.v1.placeholder("float", [None, 1])

critic_cost = tf.reduce_mean(input_tensor=tf.square(critic_target_placeholder - critic_output_layer))
critic_train_operation = tf.compat.v1.train.AdamOptimizer(LEARN_RATE_CRITIC).minimize(critic_cost)
