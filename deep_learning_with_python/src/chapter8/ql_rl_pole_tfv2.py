...
input_placeholder = tf.compat.v1.placeholder("float", [None, input_size])

weights_1 = tf.Variable(tf.random.truncated_normal([input_size, 20], stddev=0.01))
bias_1 = tf.Variable(tf.constant(0.0, shape=[20]))

weights_2 = tf.Variable(tf.random.truncated_normal([20, env.action_space.n], stddev=0.01))
...
action_placeholder = tf.compat.v1.placeholder("float", [None, 2])
target_placeholder = tf.compat.v1.placeholder("float", [None])

q_estimation = tf.reduce_sum(input_tensor=tf.multiply(output_layer, action_placeholder), axis=1)
loss = tf.reduce_mean(input_tensor=tf.square(target_placeholder - q_estimation))

train_operation = tf.compat.v1.train.AdamOptimizer().minimize(loss)
