...
def main(rank, world_size):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=3, num_workers=0, sampler=train_sampler, collate_fn=utils.collate_fn)
    ...
    epochs = 0
    checkpoint = torch.load("./models/save_model/coco_checkpoint_params{}.pth".format(epochs), map_location=str(device))

    model = coco_densenet201()
